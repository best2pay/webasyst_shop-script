 ...
 * @subpackage Payment
 * @name Best2Pay
 * @description Best2Pay payment module
 * @payment-type online
 *
 ...
 *
 */
class best2payPayment extends waPayment implements waIPayment {
	...
	public function payment($payment_form_data, $order_data, $auto_submit = false) {
		...
		$signature = base64_encode(md5($this->sector_id . $best2pay_id . $this->password));
	...
	private function checkPaymentState($request) {
		$b2p_order_id = intval($request['id']);
		if (!$b2p_order_id)
			throw new waPaymentException("Invalid order id", $code);

		$b2p_operation_id = intval($request['operation']);
		if (!$b2p_operation_id)
			throw new waPaymentException("Invalid operation id", $code);

		$signature = base64_encode(md5($this->sector_id . $b2p_order_id . $b2p_operation_id . $this->password));
